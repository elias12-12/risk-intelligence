...
**Status:** session 1 of 5 complete — 262 tests green, `alert.v1`'s digest
unmoved. Sessions 2–5 remain. Each ends with the suite green and this file
updated.
...
| D4 | **Admin-authored rules have no validation to fail against.** `0006_rules.sql` has no CHECK on `status`, `operator` or `combine`; the FK on `feature_key` is the only guardrail. `conditions.fires()` returns `False` for an unrecognised operator, so a rule authored with a typo never fires, never errors, and appears in the ledger as a condition that simply never matched | `db/migrations/0006_rules.sql`; `src/glassbox/engine/conditions.py:76` | Session 2 |
...
| # | Decision | Note |
|---|---|---|
| O1 | **The population sample cap for `/simulate/rule`** | The transaction lane is 9,844 subjects and an HTTP request should not evaluate them unbounded. Proposed: cap at 2,000, configurable, and **publish the cap as the denominator** in the response, in the same idiom as every other rate here. Unconfirmed |
...
| | Item | Notes |
|---|---|---|
| **TODO** | `contract/catalog.py` — `catalog.v1` | Read shapes for rules, conditions, catalog features and reference vocabulary |
| **TODO** | `GET /rules` | id, name, subject type, lane, action, thresholds, status, version |
| **TODO** | `GET /rules/{rule_id}` | conditions, version history from `rule_versions`, shadow/active state, and **per-condition performance from `v_condition_performance`** — fire rate, precision by direction, points per precision point |
| **TODO** | `GET /features` | The catalog: window, `max_staleness`, `default_when_absent`, `inline_capable`, `resolution_path`, `spec_version` |
| **TODO** | `GET /reference` | Operators, actions with their severity ladder, subject types, reason codes — so the UI's dropdowns and the validator's rejections agree |
| **TODO** | `rules/validate.py` — the validation layer | Shared by simulate and publish (decision 6). Rejects: unknown operator; action not in `ref_action`; subject type not in `ref_subject_type`; `feature_key` not in the catalog; `status` outside `active`/`shadow`/`inactive`; `combine` outside `AND`/`OR`; a `prevent_threshold` below its `review_threshold`; a negative contribution on a feature carrying a non-null `default_when_absent` (that is the §5 trap `test_degraded.py` already enforces across the catalog); an inline rule reading a feature that is not `inline_capable` |
| **TODO** | Collecting evaluation path | `run_lane` throws results away when `persist=False`; (b) needs them. Either a `collect=True` variant or its own loop over `evaluate_batch` |
| **TODO** | `POST /simulate/rule` — **admin only** | Insert draft → load `EngineContext` → evaluate → roll back |
| **TODO** | Aggregate response | Subjects evaluated (with the **cap as denominator**, O1), would-fire count, would-raise count, score distribution, precision against `synthetic_label`, per-condition performance, worked examples, and a diff of decisions whose action or score would change |
